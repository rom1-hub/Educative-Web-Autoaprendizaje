...
(function(){
  function loadConjugaisonExpansion(){
    ...
    const scripts=['../js/conjugaison/data-expansion.js','../js/conjugaison/family-patterns.js','../js/conjugaison/table-presentation.js','../js/conjugaison/elision-display.js'];
  ...
  function initCoqMenu(){
    document.querySelectorAll('.nav').forEach(function(nav){
      ...
      document.addEventListener('click',function(ev){
        if(window.innerWidth<=900 && links.classList.contains('mobile-open') && !nav.contains(ev.target)) closeMenu();
      });
      document.addEventListener('keydown',function(ev){
        if(ev.key==='Escape' && window.innerWidth<=900 && links.classList.contains('mobile-open')){
          closeMenu();
          button.focus();
        }
      });
  ...
  function initBlogFilters(){
    ...
    if(!filterBar || !grid || filterBar.dataset.coqFiltersReady==='1') return;
    filterBar.dataset.coqFiltersReady='1';
    const buttons=Array.from(filterBar.querySelectorAll('.blog-cat'));
    ...
    buttons.forEach(function(button){
      ...
      button.addEventListener('click',function(){
        ...
        cards.forEach(function(card){
          const label=card.querySelector('.blog-cover');
          const text=label ? label.textContent.trim().toLowerCase() : '';
          const show=category==='todos' || text.indexOf(category)!==-1;
          card.hidden=!show;
        });
      });
    });
