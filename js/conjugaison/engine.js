...
(function(){
  ...
  function generatedSimple(verb,tense,subject){
    const r=record(verb);
    if(!r||!R||typeof R.generate!=='function')return null;
    return R.generate(r.pattern,r.infinitif_base||r.infinitif,P.baseSubject(subject),tense);
  }
  ...
  function explicitForm(verb,tense,subject){
    ...
    const grouped=rows.find(r=>String(r[0]).split('/').map(x=>x.trim()).some(label=>P.baseSubject(label)===base));
    return grouped?grouped[1]:null;
  }

  function simpleForm(verb,tense,subject){
    const explicit=explicitForm(verb,tense,subject);
    if(explicit!==null&&explicit!==undefined&&String(explicit)!=='')return explicit;
  ...
  function compoundForm(verb,tense,subject,construction){
    ...
    const auxTense=C.auxiliaryTense(tense);
    const auxForm=simpleForm(auxiliary,auxTense,info.base);
    if(!auxForm)return null;
    const pp=participle(base);
    if(!pp)return null;
    ...
    if(isPronominal){
      ...
      return contracted+(contracted.endsWith("'")?'':' ')+agreed;
    }
  ...
  function lookupCompoundSubjects(tense){
    if(tense==='subjonctif passé')return [
      'que je (masculin singulier)','que je (féminin singulier)','que tu (masculin singulier)','que tu (féminin singulier)',
      "qu'il","qu'elle","qu'on (masculin singulier)","qu'on (masculin pluriel)","qu'on (féminin pluriel)",
      'que nous (masculin pluriel)','que nous (féminin pluriel)','que vous (masculin singulier)','que vous (féminin singulier)',
      'que vous (masculin pluriel)','que vous (féminin pluriel)',"qu'ils","qu'elles"
    ];
    return [
      'je (masculin singulier)','je (féminin singulier)','tu (masculin singulier)','tu (féminin singulier)',
      'il','elle','on (masculin singulier)','on (masculin pluriel)','on (féminin pluriel)',
      'nous (masculin pluriel)','nous (féminin pluriel)','vous (masculin singulier)','vous (féminin singulier)',
      'vous (masculin pluriel)','vous (féminin pluriel)','ils','elles'
    ];
  }
  ...
  function rowsForLookup(verb,tense){
    const r=record(verb);
    if(!r)return [];
    if(!(C&&C.isCompound(tense)))return rowsFor(verb,tense);
    const construction=r.pronominal?'pronominale':(r.construction||'non-pronominale');
    return lookupCompoundSubjects(tense).map(subject=>[subject,conjugate(verb,tense,subject,construction)]).filter(row=>row[1]!=null);
  }
  ...
  function rowsFor(verb,tense){
    ...
    const isSimple=C?.isSimple?.(tense);
    const isCompound=C?.isCompound?.(tense);
    if(!isSimple&&!isCompound)return source.map(x=>[x[0],x[1]]);
    const construction=r.pronominal?'pronominale':(r.construction||'non-pronominale');
    if(isCompound)return rowsForLookup(verb,tense);
    const fallback=tense==='impératif présent'?['tu','nous','vous']:tense==='subjonctif présent'?['que je','que tu',"qu'il/elle/on",'que nous','que vous',"qu'ils/elles"]:['je','tu','il/elle/on','nous','vous','ils/elles'];
    const rows=source.length?source:fallback.map(subject=>[subject,'']);
    return rows.map(row=>{
      const subject=String(row[0]).split('/').map(x=>x.trim()).filter(Boolean)[0];
      const generated=conjugate(verb,tense,subject,construction);
      return generated!=null?[row[0],generated]:null;
    }).filter(Boolean);
  }
  ...
  function rowsForConstruction(verb,tense,construction){
    ...
    const source=(r.formes||{})[tense]||[];
    if(!C?.isSimple?.(tense))return source.map(x=>[x[0],x[1]]);
  ...
  function canGenerate(verb,tense){
    const r=record(verb);
    if(!r)return false;
    if(C?.isCompound?.(tense))return !!(r.auxiliaire||r.pronominal||r.construction==='pronominale');
    return !!C?.isSimple?.(tense)&&!!R&&typeof R.generate==='function';
  }
