...
(function(){
  ...
  function conjugate(verb,tense,subject,construction){const r=record(verb);if(!r)return null;const isPronominal=constructionResolver?.isPronominal(r,construction);if(C&&C.isCompound(tense))return compoundForm(verb,tense,subject,isPronominal?'pronomiale':'non-pronomiale');let form=simpleForm(baseKey(verb),tense,subject);if(form==null)return null;if(isPronominal){if(tense==='impératif présent'){const imperativePronoun=P.imperativePronounFor?.(subject);if(!imperativePronoun)return form;return form+'-'+imperativePronoun;}form=P.apply(subject,form);}return form;}
  ...
  function rowsFor(verb,tense){const r=record(verb);if(!r)return [];const source=(r.legacyFormes||{})[tense]||[],isSimple=C?.isSimple?.(tense),isCompound=C?.isCompound?.(tense);if(!isSimple&&!isCompound)return source.map(x=>[x[0],x[1]]);const construction=constructionResolver?.isPronominal(r)?'pronomiale':'non-pronomiale';if(isCompound)return practiceCompoundSubjects(tense).map(subject=>[subject,conjugate(verb,tense,subject,construction)]).filter(row=>row[1]!=null);const fallback=tense==='impératif présent'?S.imperative:tense==='subjonctif présent'?S.subjonctifSimpleFallback:S.simpleFallback,rows=source.length?source:fallback.map(subject=>[subject,'']);return rows.map(row=>{const subject=String(row[0]).split('/').map(x=>x.trim()).filter(Boolean)[0],generated=conjugate(verb,tense,subject,construction);return generated!=null?[row[0],generated]:null;}).filter(Boolean);}
  function rowsForConstruction(verb,tense,construction){const r=record(verb);if(!r)return [];if(C?.isCompound?.(tense))return practiceCompoundSubjects(tense).map(subject=>[subject,conjugate(verb,tense,subject,construction)]).filter(row=>row[1]!=null);const source=(r.legacyFormes||{})[tense]||[];if(!C?.isSimple?.(tense))return source.map(x=>[x[0],x[1]]);const fallback=tense==='impératif présent'?S.imperative:tense==='subjonctif présent'?S.subjonctifConstructionFallback:S.simpleConstructionFallback,rows=source.length?source:fallback.map(subject=>[subject,'']);return rows.map(row=>{const subject=String(row[0]).split('/').map(x=>x.trim()).filter(Boolean)[0],generated=conjugate(verb,tense,subject,construction);return generated!=null?[row[0],generated]:null;}).filter(Boolean);}
