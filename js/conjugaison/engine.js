...
(function(){
  ...
  function rowsForLookup(verb,tense){const r=record(verb);if(!r)return [];if(!(C&&C.isCompound(tense)))return rowsFor(verb,tense);const construction=r.pronominal?'pronominale':(r.construction||'non-pronominale');const subjects=tense==='subjonctif passé'?SUBJONCTIF_PRACTICE_SUBJECTS:LOOKUP_COMPOUND_SUBJECTS;return subjects.map(subject=>[subject,conjugate(verb,tense,P.baseSubject(subject),construction)]).filter(row=>row[1]!=null);}
