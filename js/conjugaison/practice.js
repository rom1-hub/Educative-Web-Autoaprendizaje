...
(function(){
  ...
  function selectPracticeQuestions(pool,limit){
    ...
    let available=new Set(unique.map((_,index)=>index));
    let previousKey='';
    while(selected.length<target){
      if(!available.size){
        available=new Set(unique.map((_,index)=>index));
        if(available.size>1){
          const previousIndex=unique.findIndex(q=>practiceQuestionKey(q)===previousKey);
          if(previousIndex>=0)available.delete(previousIndex);
        }
      }
      let bestScore=Infinity,best=[];
      available.forEach(index=>{
        const q=unique[index],key=practiceQuestionKey(q);
      ...
      const pick={...unique[pickIndex]};
      available.delete(pickIndex);
      selected.push(pick);
      previousKey=practiceQuestionKey(pick);
  ...
  function buildQuestions(verb,tense,category,construction,auxiliary){
    ...
    const add=v=>{
      ...
      const ts=tense==='Todos los tiempos'?allTenses:[tense];
      ts.forEach(t=>{
        ...
        let rows=(engine&&engine.rowsFor)?engine.rowsFor(v,t):[];
