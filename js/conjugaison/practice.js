...
(function(){
  ...
  function sampleMatchingVerbs(target,predicate){const wanted=Math.max(0,Number(target)||0);if(!wanted)return [];const selected=[];let seen=0;for(const v of Object.keys(dataModel?.records||{})){const record=getRecord(v);if(!predicate(v,record))continue;seen+=1;if(selected.length<wanted){selected.push(v);continue;}const index=Math.floor(Math.random()*seen);if(index<wanted)selected[index]=v;}return selected;}
