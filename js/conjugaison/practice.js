...
(function(){
  ...
  const practiceGroupOptionHtml=(()=>{
    ...
    return (first?optionHtml(first):'<option value="all">Todos los grupos</option>')+sections.map(section=>'<optgroup label="'+U.escapeHtml(section)+'">'+options.filter(item=>item.section===section).map(optionHtml).join('')+'</optgroup>').join('');
  })();
  const practiceAuxiliaryOptionHtml=auxiliaryOptions.map(item=>'<option value="'+U.escapeHtml(item.id)+'">'+U.escapeHtml(item.label)+'</option>').join('');
  const practiceConstructionOptionHtml=constructionOptions.map(item=>'<option value="'+U.escapeHtml(item.id)+'">'+U.escapeHtml(item.label)+'</option>').join('');
  let session={questions:[],index:0,correct:0,results:[],locked:false};
  let practiceRecordSource=null;
  let practiceRecordKeys=[];
  function getPracticeRecordKeys(){const records=dataModel?.records||{};if(records!==practiceRecordSource){practiceRecordSource=records;practiceRecordKeys=Object.keys(records);}return practiceRecordKeys;}
  function matchesCategory(v,category,knownRecord){if(!category || category==='all')return true;return !!(categoryResolver&&typeof categoryResolver.matchesCategory==='function'&&categoryResolver.matchesCategory(v,category,knownRecord));}
  ...
  function sampleMatchingVerbs(target,predicate){const wanted=Math.max(0,Number(target)||0);if(!wanted)return [];const selected=[];let seen=0;for(const v of getPracticeRecordKeys()){const record=getRecord(v);if(!predicate(v,record))continue;seen+=1;if(selected.length<wanted){selected.push(v);continue;}const index=Math.floor(Math.random()*seen);if(index<wanted)selected[index]=v;}return selected;}
  function sampleMatchingVerbsByGroup(targets,predicate){const groups=new Map(Object.entries(targets||{}).map(([group,target])=>[Number(group),{target:Math.max(0,Number(target)||0),selected:[],seen:0}]));if(!groups.size)return [];for(const v of getPracticeRecordKeys()){const record=getRecord(v);if(!predicate(v,record))continue;const group=Number(record?.groupe);const bucket=groups.get(group);if(!bucket||!bucket.target)continue;bucket.seen+=1;if(bucket.selected.length<bucket.target){bucket.selected.push(v);continue;}const index=Math.floor(Math.random()*bucket.seen);if(index<bucket.target)bucket.selected[index]=v;}
  ...
  function buildQuestions(verb,tense,category,construction,auxiliary){
    ...
    if(verb){
      ...
      const predicate=(v,record)=>matchesCategory(v,category,record)&&matchesConstruction(record,construction);
      let selectedVerbs;
      if(category==='all'){
