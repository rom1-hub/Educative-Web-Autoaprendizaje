...
(function(){
  ...
  function buildQuestions(verb,tense,category,construction,auxiliary){
    ...
    const add=(v,requestedTenses=null)=>{
      ...
      ts.forEach(t=>{
        const isSimple=simpleTenses.includes(t),isCompound=compoundTenses.includes(t);
    ...
    if(pool.length<20&& !verb && tense==='Todos los tiempos'){
      pool=[];
      Object.keys(dataModel?.records||{}).forEach(add);
    }
