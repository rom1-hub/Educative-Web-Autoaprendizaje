...
(function(){
        ...
        U.expandPracticeRows(rows).forEach(r=>{
          ...
          if(isCompound && variants[baseSubject] && engine&&engine.conjugate){
            variants[baseSubject].forEach(subject=>{
              ...
              if(answer!=null)pushQuestion(pool,v,t,formatPracticeSubject(subject,t,isCompound),answer);
            });
          }else{
            ...
            if(finalAnswer!=null && String(finalAnswer).trim()!=='')pushQuestion(pool,v,t,formatPracticeSubject(r.subject,t,false),finalAnswer);
          }
        });
      });
    };
  ...
  function showQuestion(){const q=session.questions[session.index];session.locked=false;q.attempts=0;q.firstError='';q.secondError='';q.mustTypeCorrect=false;document.querySelector('#questionVerb').textContent=`${q.verb} · ${q.tense}`;document.querySelector('#questionSubject').textContent=q.subject;const input=document.querySelector('#answerInput');input.value='';input.className='';input.disabled=false;const fb=document.querySelector('#practiceFeedback');fb.className='feedback-box';fb.textContent='';document.querySelector('#practiceProgressText').textContent=`Question ${session.index+1} / 20`;document.querySelector('#practiceProgressBar').style.width=`${(session.index/session.questions.length)*100}%`;input.focus();}
  function finishSession(){const correct=session.correct,errors=session.questions.length-correct,score=session.questions.reduce((sum,q)=>sum+(q.score||0),0);document.dispatchEvent(new CustomEvent('coq:practice-finished',{detail:{results:session.results,correct,errors,score}}));}
  function validateAnswer(){if(session.locked)return;const q=session.questions[session.index],input=document.querySelector('#answerInput'),fb=document.querySelector('#practiceFeedback');if(!q||!input||!fb)return;const value=input.value.trim();if(q.mustTypeCorrect){if(samePracticeAnswer(value,q)){q.mustTypeCorrect=false;q.attempts=2;fb.className='feedback-box success';fb.textContent='Réponse confirmée. Tu peux continuer.';session.index++;if(session.index>=session.questions.length)finishSession();else showQuestion();}else{fb.className='feedback-box error';fb.textContent='Retape exactement la réponse correcte affichée pour continuer.';}return;}q.attempts=(q.attempts||0)+1;if(samePracticeAnswer(value,q)){q.score=q.attempts===1?1:.5;session.correct++;if(q.attempts===1){fb.className='feedback-box success';fb.textContent='Correct !';}else{fb.className='feedback-box success';fb.textContent='Correct après une première erreur : 0,5 point.';}session.results.push({question:{...q},outcome:q.attempts===1?'correct':'correct-after-first-error',finalAnswer:value});session.index++;if(session.index>=session.questions.length)finishSession();else showQuestion();return;}if(q.attempts===1){q.firstError=value;fb.className='feedback-box error';fb.textContent='Réponse incorrecte. Essaie encore.';return;}q.secondError=value;q.score=0;q.mustTypeCorrect=true;fb.className='feedback-box error';fb.innerHTML=`Réponse incorrecte deux fois.<br><strong>Réponse correcte :</strong> ${U.escapeHtml(q.displayAnswer||q.answer)}<br><span>Retape la réponse correcte pour continuer.</span>`;session.results.push({question:{...q},outcome:'incorrect-twice'});}
  function resetPracticeForm(){session={questions:[],index:0,correct:0,results:[],locked:false};document.querySelector('#practiceSession')?.classList.add('hidden');const msg=document.querySelector('#practiceMessage');if(msg){msg.className='form-message';msg.textContent='';}const fb=document.querySelector('#practiceFeedback');if(fb){fb.className='feedback-box';fb.textContent='';}updatePracticeGroupState();updatePracticeConstructionOptions();updatePracticeAuxiliaryOptions();}
  function init(){const start=document.querySelector('#startPractice'),validate=document.querySelector('#validateAnswer'),clear=document.querySelector('#clearVerb'),verbInput=document.querySelector('#practiceVerb'),tense=document.querySelector('#practiceTense'),construction=document.querySelector('#practiceConstruction');start?.addEventListener('click',startSession);validate?.addEventListener('click',validateAnswer);document.querySelector('#answerInput')?.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();validateAnswer();}});clear?.addEventListener('click',()=>{if(verbInput){verbInput.value='';verbInput.dispatchEvent(new Event('input',{bubbles:true}));verbInput.dispatchEvent(new Event('change',{bubbles:true}));}});verbInput?.addEventListener('input',()=>{updatePracticeGroupState();updatePracticeConstructionOptions();updatePracticeAuxiliaryOptions();});verbInput?.addEventListener('change',()=>{updatePracticeGroupState();updatePracticeConstructionOptions();updatePracticeAuxiliaryOptions();});tense?.addEventListener('change',updatePracticeAuxiliaryOptions);construction?.addEventListener('change',updatePracticeAuxiliaryOptions);updatePracticeGroupState();updatePracticeConstructionOptions();updatePracticeAuxiliaryOptions();}
  window.COQ_CONJ_PRACTICE={buildQuestions,startSession,validateAnswer,resetPracticeForm,samePracticeAnswer};
