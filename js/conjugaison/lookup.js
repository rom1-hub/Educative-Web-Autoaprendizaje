...
(function(){
  ...
  function toggleLabel(verb,knownMeta){const m=knownMeta||meta(verb);return m.pronominal?'Ver su forma no pronominal':'Ver su forma pronominal';}
  function rowsForTense(verb,tense,recordMeta){return engine?.rowsForLookup?engine.rowsForLookup(verb,tense,recordMeta):[];}
  ...
  function normalizedRows(rows,tense,verb,knownMeta){
    const output=[];
    const recordMeta=knownMeta||meta(verb),construction=recordMeta.pronominal?'pronomiale':'non-pronomiale';
    (rows||[]).forEach(row=>{
  ...
  function renderRowsTable(tense,rows,recordMeta){let displayRows=normalizedRows(rows,tense,window.COQ_CONJ_LOOKUP?.currentVerb||'',recordMeta);if(tense==='impératif présent')displayRows=imperativeDecoration(displayRows);let html='<div class="tense-block"><div class="tense-head"><h3>'+U.escapeHtml(tense)+'</h3><button class="btn tiny secondary" type="button" data-speak-tense="'+U.escapeHtml(tense)+'">🔊</button></div><table class="tense-table"><tbody>';displayRows.forEach(row=>{html+='<tr><td>'+U.escapeHtml(row[0])+'</td><td>'+U.escapeHtml(row[1])+'</td></tr>';});if(tense==='impératif présent')html+='<tr><td colspan="2">* En el imperativo los sujetos desaparecen. No se pronuncian, ni se escriben.</td></tr>';return html+'</tbody></table></div>';}
  function renderConjugation(verb){const result=document.querySelector('#conjResult');if(!result)return;result.classList.remove('hidden');window.COQ_CONJ_LOOKUP.currentVerb=verb;const recordMeta=meta(verb),selectedTense=document.querySelector('#lookupTense')?.value||'',other=counterpart(verb,recordMeta),translation=String(recordMeta.traduccion||'').trim();let html='<div class="card verb-summary"><span class="tag">Consulta</span><div class="conj-result-head"><div><h2 class="verb-summary-title">'+U.escapeHtml(verb)+(translation?' <span class="verb-translation">— '+U.escapeHtml(translation)+'</span>':'')+'</h2><p class="muted">'+(recordMeta.pronominal?'Forma pronominal':'Forma no pronominal')+'</p></div><div class="conj-actions">';if(other)html+='<button class="btn secondary" type="button" id="togglePronominal" data-target-verb="'+U.escapeHtml(other)+'">'+toggleLabel(verb,recordMeta)+'</button>';html+='<button class="btn secondary" type="button" id="speakVerb" aria-label="Escuchar el verbo">🔊 Escuchar el verbo</button><button class="btn tiny secondary" type="button" id="practiceThisVerb">Practicar este verbo</button></div></div></div>';if(selectedTense){let timesToShow=[];if(selectedTense==='Todos los tiempos'){timesToShow=(C?.displayOrder||[]).filter(t=>engine?.canGenerate?.(verb,t,recordMeta)).map(t=>[t]);}else if(engine?.canGenerate?.(verb,selectedTense,recordMeta))timesToShow=[[selectedTense]];const generated=timesToShow.map(([tense])=>[tense,rowsForTense(verb,tense,recordMeta)]).filter(([,rows])=>rows?.length);html+='<div class="conj-toolbar"><span class="muted">'+(selectedTense==='Todos los tiempos'?'Todos los tiempos':'Tiempo seleccionado')+'</span></div>';if(!generated.length)html+='<div class="callout">Todavía no hay una conjugación disponible para <strong>'+U.escapeHtml(verb)+'</strong> en el tiempo «'+U.escapeHtml(selectedTense)+'».</div>';else{html+='<div class="conj-times" id="conjTimes">';generated.forEach(([tense,rows])=>{html+=renderRowsTable(tense,rows,recordMeta);});html+='</div>';}}else html+='<div class="callout">Selecciona un tiempo verbal para mostrar la conjugación.</div>';result.innerHTML=html;bindResultControls(verb);}
  function bindResultControls(verb){const recordMeta=meta(verb),speak=window.Coqaudio&&window.Coqaudio.speak?window.Coqaudio.speak:function(){};addFormAudioButtons(document.querySelector('#conjResult'));document.querySelector('#speakVerb')?.addEventListener('click',()=>speak(verb));document.querySelector('#togglePronominal')?.addEventListener('click',e=>{const target=e.currentTarget.dataset.targetVerb;document.querySelector('#lookupVerb').value=target;renderConjugation(target);setLookupMessage((document.querySelector('#lookupTense')?.value==='Todos los tiempos'?'Conjugación de todos los tiempos verbales cargada para «':'Conjugación cargada para «')+target+'».','form-message info');});document.querySelectorAll('[data-speak-tense]').forEach(button=>button.addEventListener('click',()=>{const tense=button.dataset.speakTense,rows=rowsForTense(verb,tense,recordMeta),spoken=[],seen=new Set();rows.forEach(row=>{const subject=stripMetadata(row[0]).trim(),form=speechForm(row[1]);if(!form)return;if(isCompound(tense)&&seen.has(subject))return;seen.add(subject);spoken.push((subject+' '+form).trim());});speak(spoken.join('. '));}));}
