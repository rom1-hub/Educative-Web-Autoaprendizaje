...
(function(){
  ...
  const source={...base};
  Object.entries(familyCatalog).forEach(([key,record])=>{source[key]={...(source[key]||{}),...record};});
  Object.entries(constructionCatalog).forEach(([key,record])=>{source[key]={...(source[key]||{}),...record};});
  ...
  const registry={};
  Object.keys(source).forEach(key=>{const record=source[key];if(!record||typeof record!=='object')return;registry[key]=deepFreeze(clone(record));});
  deepFreeze(registry);
  const normalizedMeta={};Object.entries(registry).forEach(([key,record])=>{const meta={...record};if(record.pronominal===true)meta.auxiliaire=null;normalizedMeta[key]=deepFreeze(meta);});deepFreeze(normalizedMeta);
  const api={conjugations:registry,verbGroups:Object.freeze({...((window.COQ_VERB_DATA||{}).verbGroups||{})}),verbMeta:normalizedMeta};
  ...
  window.COQ_VERB_REGISTRY=registry;window.COQ_CONJ_UTILS=api;
})();
