 ...
 * Las formas simples se almacenan literalmente; COQ no las vuelve a calcular.
 */
(function(){
  ...
  function groupOf(t){if(t==='fin:ir')return 2;if(/:er$/.test(String(t||'')))return 1;return 3;}
  ...
  function stem(infinitif,t){
    ...
    return suffix&&infinitif.endsWith(suffix)?infinitif.slice(0,-suffix.length):infinitif;
  }

  function termText(term,ant){
    const value=term&&typeof term==='object'&&Object.prototype.hasOwnProperty.call(term,'i')?term.i:term;
    if(Array.isArray(value))return value.map(v=>ant+String(v)).join(' / ');
    return value==null?'':ant+String(value);
  }

  function formsFor(infinitif,t,conjugation){
    const pattern=conjugation?.[t];
    if(!pattern)return {};
    const ant=stem(infinitif,t),formes={};
    Object.entries(SIMPLE).forEach(([coq,[mode,tense]])=>{
      const terms=pattern?.[mode]?.[tense];
      if(!Array.isArray(terms))return;
      formes[coq]=terms.map((term,i)=>[PRONOUNS[i]||'',termText(term,ant)]);
    });
    const pp=pattern?.participle?.['past-participle'];
    if(pp){
      const first=Array.isArray(pp)?pp[0]:pp;
      const value=first&&typeof first==='object'&&Object.prototype.hasOwnProperty.call(first,'i')?first.i:first;
  ...
  function normalize(record,infinitif,conjugation){
    const t=record?.t||'',group=groupOf(t),formes=formsFor(infinitif,t,conjugation);
    const aux=Array.isArray(record?.aux)?record.aux:[record?.aux].filter(Boolean);
    return {
      id:infinitif,
      infinitif,
      infinitif_base:infinitif,
      groupe:group,
      sub_category:subCategoryOf(infinitif,t,group),
      pattern:t,
      patternId:'external:'+t,
      auxiliaire:aux[0]||null,
      auxiliaires:aux,
      pronominal:false,
      construction:'non-pronominale',
      participePasse:formes.participePasse||null,
  ...
  async function load(){
    try{
      const [vr,cr]=await Promise.all([
        fetch(VERBS_URL,{cache:'no-store'}),
        fetch(CONJ_URL,{cache:'no-store'})
      ]);
      ...
      Object.keys(external).forEach(verb=>{
        const old=local[verb]||{};
        merged[verb]={
          ...old,
          ...external[verb],
          formePronominale:external[verb].formePronominale||old.formePronominale||null,
          formeNonPronominale:external[verb].formeNonPronominale||old.formeNonPronominale||null,
          variantes:external[verb].variantes||old.variantes||null,
          exceptions:external[verb].exceptions||old.exceptions||null
        };
      });
