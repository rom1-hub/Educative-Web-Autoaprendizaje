...
expect(w.COQ_VERB_REGISTRY['venir'].auxiliaire,'être','venir doit conservar son auxiliaire être dans le registro normalizado');
...
assert(lookupRows.some(row=>row[0]==='vous (féminin singulier)'),'La consulta pronominal debe conservar las variantes de vous para la tabla.');
...
assert(venirLookupRows.length===17,`La consulta interna de venir con être debe producir 17 variantes pedagógicas; obtuvo ${venirLookupRows.length}.`);
const etreCompound=lookup.normalizedRows(venirLookupRows,'passé composé','venir');
assert(etreCompound.length===17,`Los tiempos compuestos con être deben conservar las variantes de género/número; obtuvo ${etreCompound.length}.`);
