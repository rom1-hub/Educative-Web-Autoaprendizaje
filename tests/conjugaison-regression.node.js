...
assert(lookupRows.length===9,'La consulta pronominal debe conservar los 9 sujetos separados.');
...
assert(etreCompound.length===9,'Los tiempos compuestos con être deben mostrar los 9 sujetos separados.');
...
expect(etreCompound[2][1],'est venu(e)','La tabla être debe indicar el acuerdo de il.');
expect(etreCompound[3][0],'elle','La tabla être debe separar elle.');
expect(etreCompound[3][1],'est venu(e)','La tabla être debe indicar el acuerdo de elle.');
expect(etreCompound[4][0],'on','La tabla être debe separar on.');
expect(etreCompound[4][1],'est venu(e)(s)','La tabla être debe indicar las posibilidades de on.');
expect(etreCompound[5][0],'nous','La tabla être debe usar el sujeto nous.');
expect(etreCompound[5][1],'sommes venu(e)s','La tabla être debe indicar el acuerdo plural de nous.');
expect(etreCompound[6][0],'vous','La tabla être debe usar el sujeto vous.');
expect(etreCompound[6][1],'êtes venu(e)(s)','La tabla être debe indicar las posibilidades de vous.');
expect(etreCompound[7][0],'ils','La tabla être debe separar ils.');
expect(etreCompound[7][1],'sont venu(e)s','La tabla être debe indicar el acuerdo masculino/femenino plural de ils.');
expect(etreCompound[8][0],'elles','La tabla être debe separar elles.');
expect(etreCompound[8][1],'sont venu(e)s','La tabla être debe indicar el acuerdo plural de elles.');
const subjonctifPasse=lookup.normalizedRows(engine.rowsForLookup('venir','subjonctif passé'),'subjonctif passé','venir');
assert(subjonctifPasse.length===9,'El subjonctif passé debe mostrar los 9 sujetos separados.');
...
console.log('Conjugaison regression OK — sujetos separados y reglas de acuerdo verificadas.');
