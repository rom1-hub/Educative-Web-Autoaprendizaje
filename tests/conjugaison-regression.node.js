...
expect(categories.find(g=>g.id==='er-eler')?.label,'-ELER','La categoría -ELER debe conservar son étiquette.');
...
expect(categories.find(g=>g.id==='groupe-3-aitre')?.label,'-AÎTRE','La categoría -AÎTRE debe conservar su etiqueta.');
expect(categories.find(g=>g.id==='groupe-3-eindre-aindre-oindre')?.label,'-EINDRE / -AINDRE / -OINDRE','La categoría EINDRE/AINDRE/OINDRE debe conservar su etiqueta.');
expect(categories.find(g=>g.id==='groupe-3-common')?.label,'Verbos más comunes','La categoría de verbos más comunes debe conservar su etiqueta.');
expect(categories.find(g=>g.id==='er-eler')?.familyIds?.length,2,'er-eler debe agrupar familias distintas sin usar el pattern como familia.');
expect(categories.find(g=>g.id==='yer')?.familyIds?.includes('yer-oyer-uyer'),true,'La categoría YER debe incluir -OYER/-UYER.');
expect(categories.find(g=>g.id==='yer')?.familyIds?.includes('yer-ayer'),true,'La categoría YER debe reservar una familia distinta para -AYER.');
assert(w.COQ_CATEGORY_RESOLVER.matchesCategory('appeler','er-eler'),'El resolver de categorías debe reconocer appeler en -ELER.');
...
assert(w.COQ_CATEGORY_RESOLVER.matchesCategory('aller','groupe-3-all'),'aller debe pertenecer al filtro pedagógico del tercer grupo.');
...
assert(lookupRows.length===9,'La tabla pronominal con être debe mostrar 9 sujetos.');
...
expect(subjLookup[2][0],"qu'il",'El subjonctif debe separar qu’il.');
expect(subjLookup[3][0],"qu'elle",'El subjonctif debe separar qu’elle.');
expect(subjLookup[4][0],"qu'on",'El subjonctif debe separar qu’on.');
const avoirCompound=lookup.normalizedRows(engine.rowsForLookup('avoir','passé composé'),'passé composé','avoir');
assert(avoirCompound.length===9,'Los tiempos compuestos con avoir deben mostrar 9 sujetos.');
const etreCompound=lookup.normalizedRows(engine.rowsForLookup('venir','passé composé'),'passé composé','venir');
assert(etreCompound.length===9,'Los tiempos compuestos con être deben mostrar 9 sujetos.');
const subjPasse=lookup.normalizedRows(engine.rowsForLookup('venir','subjonctif passé'),'subjonctif passé','venir');
assert(subjPasse.length===9,'El subjonctif passé con être debe mostrar 9 sujetos.');
console.log('Conjugaison regression OK — catálogo pedagógico actual, familias/patterns, concordancia y separación de sujetos verificadas.');
